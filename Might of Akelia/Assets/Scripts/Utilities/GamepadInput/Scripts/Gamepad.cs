...
public class Gamepad
{
    ...
    public bool GetButton(GamepadButtons button)
    {
        return GetButton(GamepadManager.Instance.GamepadButton(button));
    }
    ...
    {
        return GetButtonDown(GamepadManager.Instance.GamepadButton(button));
    }
    // Return button state - on CURRENT frame -- checks previous and current states
    ...
    {
        return GetButtonUp(GamepadManager.Instance.GamepadButton(button));
    }
    ...
    public bool IsMovingEitherThumbStick { get { return GetStick_L().X != 0 || GetStick_L().Y != 0 || GetStick_R().X != 0 || GetStick_R().Y != 0; } }
