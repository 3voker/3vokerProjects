...
public class DetectInteractableObject : MonoBehaviour
{
    ...
    [SerializeField]
    [Range(0.0f, 5f)] //Originally  [Range(0.0f, 0.45f)]
    private float castRadius = 0.35f;

    [Tooltip("Distance of the CapsuleCast, max = 2.0f")]
    [SerializeField]
    [Range(0.0f, 10f)] //Originally [Range(0.0f, 2.0f)]
    ...
    private IInteractable GetSuitableInteract(GameObject toCheck)
    {
        if (!IsBlocked(toCheck))
        {
            ...
            //meshGenerator = toCheck.GetComponent<MeshGeneration>();
            //meshGenerator.Createmesh(toCheck.transform.position);
#endif
