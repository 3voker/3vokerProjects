...
/// </summary>
public class DetectGrindableObject : MonoBehaviour {
    ...
    [Tooltip("Shift position of spherecast.")]
    [SerializeField]
    private float Offset = 5;
    #endregion

    #region Private fields
    private IGrindable objectToGrindOn; // reference to best IInteractable candidate
    private FollowPath objectToFollow; // reference to best IInteractable candidate

    private RaycastHit[] allHits; // array that will hold all object gathered by spherecast every frame
    public static event Action<IGrindable> ObjectToGrindOnChanged;
    public static event Action<FollowPath> OnObjectToFollowChanged;
    #endregion

    // property: sets and pass reference of IInteractable object to interact with
    private IGrindable ObjectToGrindOn
    {
        get { return objectToGrindOn; }
        set
        {
            if (objectToGrindOn != value)
            {
                objectToGrindOn = value;
                ObjectToGrindOnChanged.Invoke(objectToGrindOn);
            }
        }
    }
    // property: sets and pass reference of FollowPath object to interact with
    private FollowPath ObjectToFollow
    {
        get { return objectToFollow; }
        set
        {
            if (objectToFollow != value)
            {
                objectToFollow = value;
                OnObjectToFollowChanged.Invoke(objectToFollow);
            }
        }
    }
    void FixedUpdate()
    {
        ...
        switch (allHits.Length) // 
        {
            ...
            case 1:
                ...
                ObjectToGrindOn = GetSuitableInteract(onlyCollided);
                break;
            #endregion edgecase2
            #region default case call: if edgecase1&2* was passed (more than one object collided with spherecast)
            default:
                ObjectToGrindOn = GetSuitableGrind(); // cycle through the list to find suitable interact. null if none found.
                break;
    ...
    private RaycastHit[] QuerySphereCastHits()
    {
        return Physics.SphereCastAll(this.transform.position, castRadius,
                                        -this.transform.up, castDistance);
    }
    #endregion
    /// <summary>
    ...
    /// </returns>
    #region raycast check for obstacles: check if the Terrain is suitable for traversing
    private IGrindable GetSuitableInteract(GameObject toCheck)
    {
...
#endif
            return toCheck.GetComponent<IGrindable>();
        }
        else
            return null;
    }
    ...
    private IGrindable GetSuitableGrind(IGrindable prevCandidate = null, int prevAngle = 180, int index = 0)
    {
        ...
        if (toCompare == null) // ignore anglecheck if toCompare wasn't suitable
            return GetSuitableGrind(prevCandidate, prevAngle, index + 1);
        ...
        // if theres nothing to compare or current object is more closer to center by comparativeAngle,
            ...
            return GetSuitableGrind(toCompare, angle, index + 1); // override the previous Interact candidate
