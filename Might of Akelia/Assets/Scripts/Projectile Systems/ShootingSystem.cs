...
public class ShootingSystem : MonoBehaviour {
	...
	void Start(){
        ...
        target = gameObject.GetComponent<CombatController>().transform;
	}		
	void Update(){
	...
	public void Fire(){ //without Hit Effect
		...
		if (ammo > 0 && time > fireDelay) {	
			...
			fireMuzzle.Play ();
			time = 0;
			enemyController._Audio.Play_Fire ();
		} else {
			enemyController._Audio.Play_OutOfAmmo ();
		}
	}
	...
	public void Reload(){
		reloading = true;
		enemyController._Audio.Play_Reload ();
