...
public class RaycastHandler : MonoBehaviour {
	...
	void FixedUpdate(){
		...
		if (startPoint && lineRenderer && !enemyController._Health.isDestroyed) {
		
			RaycastHit hit;

			float range = enemyController._Shooting.range; //get range from the ShootingSystem script
			...
			if (Physics.Raycast (startPoint.position, startPoint.forward, out hit, range)) {
                ...
                enemyController._Shooting.Fire (hit.point, hit.collider.gameObject);//if hit some point then shoot through shootingSystem Fire Function
